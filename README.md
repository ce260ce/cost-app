...
2. 各カテゴリでは複数明細行を追加でき、カテゴリ内で合計値をリアルタイム表示。
3. すべてのカテゴリ入力を終えたら「計算サマリ」画面で合計原価、1ユニットあたり原価、任意の販売価格に基づく利益率を算出。
4. 後からカテゴリや項目を編集できるよう、計算サマリは常に最新の入力データを反映。
...
- **原価確認ページ**: 商品別の原価サマリ。ページ内に商品登録フォーム（基本情報/想定生産量/設備利用設定等）を配置し、登録後にカテゴリ別入力へのリンクを表示。
