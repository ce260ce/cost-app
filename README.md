...
- **materials**: `id`, `name`, `unit`, `size_description`, `default_currency`, `notes`, `created_at`, `updated_at`。
...
|カテゴリ|概要|必要になりそうな入力例|
|---|---|---|
|材料費|製品を構成する主要素材の調達コスト。為替や仕入れ単価の変動もありえる。|素材名、単価、使用量、歩留まり率|
...
## 現状の課題とTODO
- 実装では商品登録とは別に「商品別原価入力」カードを用意しており、README で定義した「商品登録フォーム内で原価入力まで完了させる」設計と乖離している。フォーム構成を統合し、参照タブに入力要素を置かないよう改修する。
- 材料マスタの仕様（仕入先、購入ロット、使用率/残量など）が UI に反映されていない。商品登録時にこれらを入力できるフィールドを追加する。
- 設備利用・電気代など、商品登録で既に取得する情報が原価入力カードでも再入力になっている。商品登録時の情報をそのまま配賦計算に使用するようデータフローを整理する。
...
- **製品単価への載せ方**: 年次按分額を対象商品の想定生産数量で割り、商品 1 ユニットあたりに配賦。生産数量の詳細定義は後続設計で詰める。
- **拡張余地**: フェーズ別（試作1号/量産前）に分ける、償却残期間の管理、途中での追加開発費の追加入力など。

## 設備投資の扱い
- **必要入力**: 取得額（税抜ベース推奨）、償却年数、想定年間生産数量（年間で何ユニット生産するか）。想定生産量は商品登録時の入力から自動で候補を表示し、設備ごとに配分割合を設定できるようにする。
- **年間償却額**: `取得額 ÷ 償却年数`。減価償却方法は定額法を基本とし、必要であれば今後カスタマイズ。
- **ユニットあたり設備コスト**: `年間償却額 ÷ 想定年間生産数量`。これを材料原価に加算する形で原価シートへ組み込む。複数商品の生産を同じ設備で行う場合、設備ごとの年間生産数量は商品別計画の合算値になるため、種類が増えるほどユニットコストは下がる。
- **計算タイミング**: 生産数量を更新した場合はユニットコストを再計算。年間数量が未入力なら仮で 1 とし、警告を表示。
- **拡張ポイント**: 複数設備をまとめて管理し、製品ごとに利用比率（例: 設備 A を 60% 使用）で按分する機能を後から追加可能。
...
- **サイズ展開**: 同一商品内で複数サイズがある場合の一覧。各サイズに固有のコストを紐づけるかは後で設計。
...
- **オプションの種類**: 任意入力。金具変更などユーザーが選べる派生を列挙し、追加コストのテンプレートに使えるようにする。
